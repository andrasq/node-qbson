...
var Decimal128 = bsonTypes.Decimal128;
...
/**
// interesting... with newer node, Math.log is faster
function clenIndex( i ) {
    if (i < 10) return 1;
    if (i < 100) return 2;
    for (var n = 3, lim = 1000; i >= lim; n += 1, limit *= 10) ;
    return n;
}
**/
...
// compound items are preceded by 4-byte length and ended with 1-byte NUL, and consist of typeId + cname + NUL + <contens>
function guessCompoundSize( item ) {
    ...
    if (Array.isArray(item)) {
        for (i=0; i<item.length; i++) {
            ...
            contentsSize += (1) + ((i < 9) ? (1 + 1) : (Math.ceil(Math.log(i + 2) / Math.log(10)) + 1)) + guessSize(item[i]);
            //contentsSize += (1) + clenIndex(i) + 1 + guessSize(item[i]);
        }
    }
    else {
        for (key in item) {
