...
function parseItemObject( info ) {
    ...
    default:
        if (hasSymbol && info.v.constructor === Symbol) {
            info.v = String(info.v);
...
/**
    ...
    case Boolean: return T_STRING;
    case Array: return T_ARRAY;
    default: return value ? determineClassTypeId(value) : (value === null) ? T_NULL : T_UNDEFINED;
    }
**/
    switch (typeof value) {
    case 'number': return ((value >> 0) === value && value !== -0) ? T_INT : T_FLOAT; // also NaN and +/- Infinity
...
function determineClassTypeId( value ) {
    ...
    default:
        if (hasSymbol && value.constructor === Symbol) return T_SYMBOL;
        return T_OBJECT;
    }
}
