...
function utf8_byteLength( buf, pos, str, base, bound, asOverlong ) {
    base = base || 0;
    bound = bound != null ? bound : str.length;
    for (var ix=0, i=base; i<bound; i++) {
        var ch = str.charCodeAt(i);
        if (ch < 0x80) {
            if (!ch && asOverlong) { ix += 2 }
            else { ix += 1 }
        }
        else if (ch < 0x800) { ix += 2 }
        else if (ch < 0xD800 || ch >= 0xE000) { ix += 3 }
        else {
            var ch2 = str.charCodeAt(i+1);
            if (ch >= 0xDC00 || !(ch2 > 0xDC00 && ch2 < 0xE000)) { ix += 3; continue; }
            ix += 4;
            i++;
        }
    }
