...
// use our own exclusive copies of these functions, do not let other users deoptimize them
var float = requireExclusive('ieee-float');
var utf8 = requireExclusive('./utf8-2');
...
// Pre-convert negative values to twos-complement, because casting float to int
// converts a negative sign-magnitude as ~x + 1 and introduces an off-by-1 into the high word
// (the +1 in the high word is needed only if the low word +1 had a carry out, ie was zero).
// 
// -x = ~x + 1, so to get the high bits of a large float:
//   convert to positive, x = -x
//   split into positive lo and hi halves
//   take the twos complement of each
//   add 1 to lo, if there is a carry out add 1 to hi
// There will be a carry out from lo iff it starts out as 0: only ~0 + 1 generates a carry
var _2e32 = 0x100000000;
function putInt64( n, target, offset ) {
    // convert sign-magnitude float n to twos-complement int64 to get the lo and hi words
    if (n >= 0) {
        var lo = n & 0xFFFFFFFF, hi = n / 0x100000000;
    } else {
        var lo = n & 0xFFFFFFFF, hi = ~((-n) / 0x100000000);    // twos complement hi and lo halves
        if (lo == 0) hi += 1;
    }
...
function requireExclusive(name) {
    delete require.cache[require.resolve(name)];
    var ret = require(name);
    delete require.cache[require.resolve(name)];
    return ret;
}
