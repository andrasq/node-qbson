...
function typeIds() { return {
    ...
    T_DECIMAL128: 19,           // 128-bit LE IEEE 754 float (new in mongodb 3.4)
...
function typeInfo() { var ids = typeIds(); return [
    ...
    { id: ids.T_DECIMAL128,     key: 'T_DECIMAL128',    name: 'Decimal128',     size: 16,  fixup: 0 },
 ...
 * We can create, read and write objects of this type, thats it.
 * It is stored as a type2 string (byteLength + refname + \0) then 12 bytes of objectid
 * The mongo shell DBRef() takes takes exactly two arguments, ref and id; no db.
 */
...
// 128-bit ieee 754 little-endian floating point value
// We can create, read and write this object, but not convert it to/from a native number.
function Decimal128( hi, hi2, hi3, lo ) {
    this.hi = hi;
    this.hi2 = hi2;
    this.hi3 = hi3;
    this.lo = lo;
