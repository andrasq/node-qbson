...
var determineTypeId = require('./bson-size.js').determineTypeId;
var guessSize = require('./bson-size').guessSize;
var newBuffer = require('./new-buffer');
...
//var size = 1000;
    var buf = newBuffer.alloc(size);

//var buf = newBuffer.alloc(1000);
    //var info = parseItem('', obj);
//console.log(info);
    //var buf = newBuffer.alloc(info.sz);
    ...
    //var ret = newBuffer.alloc(offset);
    //buf.copy(ret);
    //return ret;
}
...
var T_DECIMAL128 = 19;  // 128-bit LE IEEE 754 float (added in mongod 3.4)
...
function encodeEntity( name, value, target, offset ) {
    ...
    case T_SCOPED_CODE:
        var mark = offset;
        offset = putString(String(value), target, offset + 4);
        offset = encodeEntities(value.scope, target, offset);
        putInt32(offset - mark, target, mark);
        break;
    case T_DECIMAL128:
        offset = putInt32(value.lo, target, offset);
        offset = putInt32(value.hi3, target, offset);
        offset = putInt32(value.hi2, target, offset);
        offset = putInt32(value.hi, target, offset);
        break;
